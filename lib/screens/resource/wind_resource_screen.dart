  ...
  void _handleQuery() async {
    if (_latController.text.isEmpty || _lngController.text.isEmpty) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('请输入经纬度')));
      return;
    }
    ...
    try {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('查询失败')));
            ...
            if (_result != null) ...[
              ...
              GridView.count(
                ...
                children: [
                  _buildDataCard(
                    '风速年均',
                    _result!.windSpeedAnnual.toStringAsFixed(2),
                    'm/s',
                  ),
                  _buildDataCard(
                    '风能密度',
                    _result!.windPowerDensity.toStringAsFixed(1),
                    'W/m²',
                  ),
                  _buildDataCard(
                    'Weibull k',
                    _result!.weibullK.toStringAsFixed(2),
                    '-',
                  ),
                  _buildDataCard(
                    '湍流强度',
                    (_result!.turbulenceIntensity * 100).toStringAsFixed(1),
                    '%',
                  ),
                ],
              ),
